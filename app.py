...
def user_input_features():
    ...
    reamining_contract = st.sidebar.number_input("Залишок контракту/reamining_contract", min_value=0.0, max_value=100.0, step=0.01, key='reamining_contract')
    ...
    download_avg = st.sidebar.number_input("Середня швидкість завантаження/download_avg", min_value=0.0, max_value=100.0, step=0.01, key='download_avg')
    ...
    download_over_limit = st.sidebar.selectbox("Чи було перевищення ліміту завантаження?/download_over_limit", [0, 1], key='download_over_limit')
